...
export default function EditRestaurantScreen ({ navigation, route }) {
  ...
  const [isPressed, setIsPressed] = useState(false) // Permite que, tras pulsar, haga un efecto de color, para saber que realmente hemos pulsado un botón.
  ...
  const [initialRestaurantValues, setInitialRestaurantValues] = useState({ name: null, messageToFans: null, description: null, address: null, postalCode: null, url: null, shippingCosts: null, email: null, phone: null, restaurantCategoryId: null, logo: null, heroImage: null, discountCode: null, discount: null, promoted: false })
  ...
  useEffect(() => {
    async function fetchRestaurantDetail () { // Función que busca y obtiene los detalles de un restaurante específico, realiza preparaciones en los datos obtenidos y establece valores iniciales.
      try {
  ...
  const updateRestaurant = async (values) => { // Función que actualiza la información de un restaurante existente, muestra mensajes de éxito o errores y realiza la navegación correspondiente.
    // Esta función se ejecutará cuando el usuario presione el botón enviar/guardar al final del formulario.
    setBackendErrors([])
    try {
  ...
  return (
    <Formik
      enableReinitialize // Propiedad que indica si se deben reinicializar los valores iniciales del formulario cuando cambian las propiedades "initialValues" o "validationSchema"
      validationSchema={validationSchema}
      initialValues={initialRestaurantValues} // Proporciona los valores iniciales del formulario.
      // Función que se ejecuta cuando se envía el formulario
      onSubmit={updateRestaurant}>
      {({ handleSubmit, setFieldValue, values }) => (
        /*
        {({ handleSubmit, setFieldValue, values }) => ...}: Esta es una función "render prop" que recibe varios objetos y funciones relacionados con el estado y el manejo del formulario.
        --> "handleSubmit" se utiliza para activar la validación y el envío del formulario.
        --> "setFieldValue" se utiliza para actualizar manualmente los valores de los campos del formulario.
        --> "values" contiene los valores actuales de todos los campos del formulario.
        */
        <ScrollView>
          <View style={{ alignItems: 'center' }}>
            <View style={{ width: '60%' }}>
              ...
              />
              <Pressable
                onPress={updateRestaurantCategories}
                onPressIn={handlePressIn}
                onPressOut={handlePressOut}
                style={({ pressed }) => [
                  styles.button,
                  { opacity: isPressed ? 0.5 : 1, backgroundColor: pressed ? GlobalStyles.brandGreenTap : GlobalStyles.brandGreen }
                ]}
              >
